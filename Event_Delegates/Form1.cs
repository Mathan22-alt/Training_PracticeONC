namespace Event_Delegates
{
    ...
    public partial class Form1 : Form
    {
        ...
        {
            SalaryCalculation dedl = delegate (int bs)
            {
                int bonus = 0;
                if (bs > 0 && bs < 20000)
                {
                    bonus = Convert.ToInt32(bs * .10);
                }
                else if (bs > 0 && bs < 50000)
                {
                    bonus = Convert.ToInt32(bs * .05);
                }
                else
                {
                    bonus = 0;
            ...
            int bonusalary = dedl(Convert.ToInt32(txtinput.Text));
            MessageBox.Show($"Salary: {txtinput.Text}, Bonus: {bonusalary}");
